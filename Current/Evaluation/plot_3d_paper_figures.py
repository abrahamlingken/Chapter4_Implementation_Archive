...
CASE_PARAMS = {
    'B': {'kappa': 0.5, 'sigma_s': 4.5, 'g': 0.0},
    'C': {'kappa': 0.5, 'sigma_s': 4.5, 'g': 0.8},
}
...
def validate_fmc_metadata(npy_file, case_key):
    """Reject stale or post-processed FMC files when metadata is available."""
    meta_file = npy_file.replace('.npy', '_meta.npz')
    if not os.path.exists(meta_file):
        return

    meta = np.load(meta_file)
    expected = CASE_PARAMS[case_key]
    ...
    for key in ('kappa', 'sigma_s', 'g'):
        if key in meta.files and not np.isclose(float(meta[key]), expected[key]):
            raise ValueError(
                ...
                f"{key}={float(meta[key])} but expected {expected[key]} ({meta_file})"
            )

    if 'postprocessed' in meta.files and bool(meta['postprocessed']):
        raise ValueError(f"Post-processed FMC benchmark is not allowed: {meta_file}")

    if 'symmetry_enforced' in meta.files and bool(meta['symmetry_enforced']):
        raise ValueError(f"Symmetry-enforced FMC benchmark is not allowed: {meta_file}")

    if 'gaussian_sigma' in meta.files and float(meta['gaussian_sigma']) > 0.0:
        raise ValueError(f"Gaussian-smoothed FMC benchmark is not allowed: {meta_file}")
...
def main():
    ...
    # FMC files: prefer raw HighStats, then UltraStats, then standard.
    
    configs = {
        'Case B (Isotropic)': {
            'model_folder': os.path.join(PROJECT_ROOT, 'Results_3D_CaseB'),
            'fmc_file': '../Solvers/MC/MC3D_Results/FMC_G_3D_CaseB_FIXED_HighStats.npy',
            'alt_fmc_files': [
                '../Solvers/MC/MC3D_Results/FMC_G_3D_CaseB_FIXED_UltraStats.npy',
        ...
        'Case C (Anisotropic)': {
            ...
            'fmc_file': '../Solvers/MC/MC3D_Results/FMC_G_3D_CaseC_FIXED_HighStats.npy',
            'alt_fmc_files': [
                '../Solvers/MC/MC3D_Results/FMC_G_3D_CaseC_FIXED_UltraStats.npy',
    ...
    for case_name, config in configs.items():
        ...
        # Load FMC benchmark
        validate_fmc_metadata(fmc_file, 'B' if 'Case B' in case_name else 'C')
